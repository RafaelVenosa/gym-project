...
@Component({
  selector: 'app-admin-registration-layout',
  standalone: true,
  imports: [
    FormsModule,
    MatFormFieldModule,
  ...
  templateUrl: './admin-registration-layout.component.html',
  styleUrl: './admin-registration-layout.component.scss'
})
export class AdminRegistrationLayoutComponent {
  ...
  enviarDados() {
    if (confirm('Tem certeza de que deseja enviar os dados?')) {
      this.dataService.registerAdmin(this.data).subscribe(
        (response: any) => {
