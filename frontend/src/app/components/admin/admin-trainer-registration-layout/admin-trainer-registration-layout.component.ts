...
@Component({
  selector: 'app-admin-trainer-registration-layout',
  standalone: true,
  imports: [
    FormsModule,
    MatFormFieldModule,
  ...
  templateUrl: './admin-trainer-registration-layout.component.html',
  styleUrl: './admin-trainer-registration-layout.component.scss'
})
export class AdminTrainerRegistrationLayoutComponent {
  ...
  enviarDados() {
    if (confirm('Tem certeza de que deseja enviar os dados?')) {
      this.dataService.registerInstructor(this.data).subscribe(
        (response: any) => {
